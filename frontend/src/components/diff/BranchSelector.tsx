  ...
  return (
    <div ref={dropdownRef} className="relative">
      ...
      >
        ...
        <span className={isDisabled ? 'text-gray-600' : 'text-gray-400'}>
          {isOpen ? '▲' : '▼'}
        </span>
      </button>
      ...
      {isOpen && (
        <div className="absolute top-full left-0 mt-1 min-w-[200px] max-h-[300px] overflow-auto bg-gray-800 border border-gray-600 rounded shadow-lg z-50">
          {isLoading && (
            <div className="px-3 py-2 text-sm text-gray-400">Switching...</div>
          )}
          {error && (
            <div className="px-3 py-2 text-sm text-red-400 border-b border-gray-700">
              {error}
            </div>
          )}
          ...
          {branchData.local.length > 0 && (
            <>
              <div className="px-3 py-1 text-xs text-gray-500 uppercase bg-gray-900">
                Local
              </div>
