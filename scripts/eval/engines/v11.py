"""Engine v11: Project context — inject tech stack & description into system prompt.

Research ("Context Conquers Parameters", ICSE 2025) shows project context
significantly improves commit message quality. Inject a condensed project
description so the model knows what it's working with.
"""
...
VERSION = "v11"
DESCRIPTION = "v6 + project context (tech stack, structure) in system prompt"
PARENT = "v6"
...
SYSTEM_PROMPT = """\
You are a commit message generator for the "Lumbergh" project.

PROJECT CONTEXT:
- Lumbergh is a self-hosted web dashboard for supervising multiple Claude Code AI sessions
- Backend: Python 3.11+, FastAPI, libtmux, TinyDB (in backend/)
- Frontend: React + Vite + TypeScript, xterm.js, TailwindCSS (in frontend/src/)
- Tests: pytest (in test/)
- Key areas: terminal streaming (WebSocket), git diff viewing, AI features, session management
